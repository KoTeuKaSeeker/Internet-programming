...
if(empty($error)) {
    ...
    if (!move_uploaded_file($_FILES['user_avatar']['tmp_name'], "../" . $path)) {
        $path = "images/defaultAvatar.jpg";
    }

    $password = md5($password);
    //Создание записи в базе данных
    mysqli_query($connect, "UPDATE `users` SET `login`='$login', `rights`='$rights', `password`='$password', `FIO`='$FIO', `avatar_path`='$path' WHERE id_user='$idEditingUser'");
